![](https://pic2.zhimg.com/v2-6d0e685c606945d7967b8afe7eef2a2b_1440w.jpg)

## 1. 数据处理

[Chardet](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Chardet&zhida_source=entity)字符编码探测器，可以自动检测文本、网页、xml的编码。

[colorama](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=colorama&zhida_source=entity)主要用来给文本添加各种颜色，并且非常简单易用。

[Prettytable](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Prettytable&zhida_source=entity)主要用于在终端或浏览器端构建格式化的输出。

[difflib](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=difflib&zhida_source=entity)，[Python]标准库，计算文本差异

[Levenshtein](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Levenshtein&zhida_source=entity)，快速计算字符串相似度。

fuzzywuzzy,字符串模糊匹配。

[esmre](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=esmre&zhida_source=entity),正则表达式的加速器。

[shortuuid](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=shortuuid&zhida_source=entity),一组简洁URL/UUID函数库。

[ftfy](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=ftfy&zhida_source=entity)，Unicode文本工具7

unidecode，ascii和Unicode文本转换函数。

[xpinyin](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=xpinyin&zhida_source=entity)，将汉字转换为拼音的函数库

[pangu.py](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pangu.py&zhida_source=entity)，调整对中日韩文字当中的字母、数字间距。

pyfiglet，Python写的figlet程序，使用字符组成ASCII艺术图片

[uniout](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=uniout&zhida_source=entity)，提取字符串中可读写的字符

[awesome slugify](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=awesome+slugify&zhida_source=entity)，一个Python slugify库，用于处理Unicode。

[python-slugify](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=python-slugify&zhida_source=entity)，转换Unicode为ASCII内码的slugify函数库。

[unicode-slugify](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=unicode-slugify&zhida_source=entity)，生成unicode内码，Django的依赖包。

ply，Python版的lex和yacc的解析工具

[phonenumbers](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=phonenumbers&zhida_source=entity)，解析电话号码，格式，存储和验证的国际电话号码。

python-user-agents，浏览器的用户代理（user-agents）的解析器。

[sqlparse](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=sqlparse&zhida_source=entity)，SQL解析器。

[pygments](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pygments&zhida_source=entity)，一个通用的语法高亮工具。

[python-nameparser](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=python-nameparser&zhida_source=entity)，解析人名，分解为单独的成分。

[pyparsing](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pyparsing&zhida_source=entity)，通用解析器生成框架。

[tablib](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=tablib&zhida_source=entity)，表格数据格式，包括，XLS、CSV，JSON，YAML。

[python-docx](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=python-docx&zhida_source=entity)，docx文档读取，查询和修改，微软Word 2007 / 2008的docx文件。
...
[csvkit](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=csvkit&zhida_source=entity)，CSV文件工具包。

[marmir](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=marmir&zhida_source=entity)，把Python[数据结构]，转化为电子表格。

[pdfminer](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pdfminer&zhida_source=entity)，从PDF文件中提取信息。

[pypdf2](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pypdf2&zhida_source=entity)， 合并和转换PDF页面的函数库。

[Python-Markdown](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Python-Markdown&zhida_source=entity)，轻量级标记语言Markdown的Python实现。

[Mistune](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Mistune&zhida_source=entity)，,快速、全功能的纯Python编写的Markdown解释器。

[dateutil](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=dateutil&zhida_source=entity)，标准的Python官方datetime模块的扩展包，字符串日期工具，其中parser是根据字符串解析成datetime，而rrule是则是根据定义的规则来生成datetime。

[arrow](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=arrow&zhida_source=entity),更好的日期和时间处理Python库

[chronyk](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=chronyk&zhida_source=entity)，一个Python 3版函数库，用于解析人写的时间和日期。

[delorean](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=delorean&zhida_source=entity)，清理期时间的函数库。

[when.py](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=when.py&zhida_source=entity)，为见的日期和时间，提供人性化的功能。

[moment](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=moment&zhida_source=entity)，类似Moment.js的日期/时间Python库

[pytz](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pytz&zhida_source=entity)，世界时区，使用tz database时区信息[数据库]

[BeautifulSoup](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=BeautifulSoup&zhida_source=entity)，基于Python的HTML/XML解析器，简单易用, 功能很强大,即使是有bug，有问题的html代码，也可以解析
...
[pyquery](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pyquery&zhida_source=entity)，类似[jQuery]的的HTML解释器函数库。

[cssutils](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=cssutils&zhida_source=entity)，Python CSS库。

[MarkupSafe](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=MarkupSafe&zhida_source=entity)，XML或HTML / XHTML安全字符串标记工具。

cssutils - ACSS library for Python., MarkupSafe - Implements a XML/HTML/XHTML [bleach](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=bleach&zhida_source=entity)，漂白，基于HTML的白名单函数库。
...
库名称简介[Mimetypes](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Mimetypes&zhida_source=entity)，Python标准库，映射文件名到MIME类型。

[imghdr](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=imghdr&zhida_source=entity)，Python标准库，确定图像类型。python-magic，libmagic文件类型识别库，Python接口格式。[path.py](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=path.py&zhida_source=entity)，os.path模块的二次封装。

[watchdog](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=watchdog&zhida_source=entity)，一组API和shell实用程序，用于监视文件系统事件。

[Unipath](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=Unipath&zhida_source=entity)，面向对象的文件/目录的操作工具包。[pathlib](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pathlib&zhida_source=entity)，-（Python 3.4版已经作为Python标准库），一个跨平台，面向path的函数库。

[pickle/cPickle](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=pickle%2FcPickle&zhida_source=entity),python的pickle模块实现了基本的数据序列和反序列化。通过pickle模块的序列化操作我们能够将程序中运行的对象信息保存到文件中去，永久存储；通过pickle模块的反序列化操作，我们能够从文件中创建上一次程序保存的对象。

cPickle是[C语言]实现的版本，速度更快。

[ConfigParser](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=ConfigParser&zhida_source=entity)，Python标准库，INI文件解析器。

[configobj](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=configobj&zhida_source=entity)，INI文件解析器。config，分层次配置，[logging](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=logging&zhida_source=entity)作者编写。

[profig](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=profig&zhida_source=entity)，多格式配置转换工具。

logging，Python标准库，日志文件生成管理函数库。

[logbook](https://zhida.zhihu.com/search?content_id=167844846&content_type=Article&match_order=1&q=logbook&zhida_source=entity)，logging的替换品。
