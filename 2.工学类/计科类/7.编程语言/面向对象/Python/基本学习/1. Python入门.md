...
1.  **`append(item)`**
    ...
    *   **示例**：
		...
		**3. 向列表中添加元组**
		...
		与列表类似，如果 `append()` 的参数是一个元组，那么会将整个元组作为一个元素添加到原列表中。
...
当比较字符串类型的数字（例如 `"33"` 和 `"22"`) 时，Python 不会报错，但比较的并非数值大小，而是字符串的字典序（按字符逐位比较）。

*   `"33" >= "22"` 的结果为 `True`，因为 `"3"` 大于 `"2"`。
*   但 `"100" >= "22"` 的结果为 `False`，因为 `"1"` 小于 `"2"`（仅比较第一个字符）。

这种比较方式并非基于数值大小，容易导致逻辑错误。

**正确做法：强制类型转换**

为了确保进行数值比较，应先将字符串转换为 `float` 或 `int` 类型：
...
num3 = int("100")
num4 = int("22")
print(num3 >= num4)  # 输出 True
```

**总结：**

*   当比较的双方都是字符串时，Python 不会抛出类型错误，但会按照字典序进行比较，这可能导致不符合预期的结果。
*   为了进行正确的数值比较，强烈建议在比较前将字符串强制转换为 `float` 或 `int` 类型。
...
另外在 Vscode 相关插件安装齐全的情况下，Vscode 的 requirements.txt 会有额外的信息提示符号（如下图）

![](https://i0.hdslb.com/bfs/openplatform/053f8e9c53af6964ffe7f1249f43d9a06b2e61ee.png)

 1. **符号含义**
	
	- **`!!!`**：当前版本存在 **已知安全漏洞** 或 **官方建议更新**（如 `pandas==1.5.3 !!!`、`matplotlib==3.10.3 !!!`）。
	
	- **`✔️`**：当前版本 **安全 / 兼容**（如 `numpy==1.26.4 ✔️`、`torchaudio==2.0.1 ✔️`）。
	- **`×`**：当前版本 **不兼容** 或 **存在严重问题**，旁边的版本是 **建议更新的目标版本**（如 `torch==2.0.0 × 2.3.2`，建议升级到 `2.3.2`；`packaging==23.2 × 25.0`，建议升级到 `25.0`）。
	- **红色三角形**：可能是 **警告提示**（如 `torch==2.0.0` 旁的三角形，强调版本问题的严重性）。
	
 2. **核心信息**:截图工具（如 `pip-audit`、IDE 插件等）扫描了项目的 `requirements.txt` 或虚拟环境，输出以下结论：
	
	- **高风险依赖**：`pandas`、`pyinstaller`、`matplotlib`、`scipy`、`hypothesis`、`onnxscript` 等库的当前版本存在安全隐患，需优先更新。
	- **兼容性问题**：`torch`、`torchvision`、`paddlepaddle`、`packaging` 等库的版本与项目环境（或官方推荐）不兼容，需升级到右侧建议版本。
	- **安全版本**：`numpy`、`torchaudio`、`paddleocr`、`tensorboard`、`expecttest` 等库的当前版本符合安全 / 兼容要求。

 3. **行动建议**
	
	1. 优先处理 **`!!!` 和 `×` 标记的依赖**，通过 `pip install <库名>==<建议版本>` 更新（如 `pip install pandas==[安全版本]`，需查官方文档确认安全版本）。
	2. 定期扫描依赖（如用 `pip-audit`、`safety` 工具），避免漏洞积累。

  

简言之，这是一份 **依赖安全体检报告**，核心目的是推动开发者修复依赖的版本问题，保障项目稳定性和安全性。
...
```ad-note
title: .whl 文件名解读
NumPy 的 `.whl` 文件后缀包含了关键的版本兼容信息，这些信息能帮助你准确选择适合自己系统和 Python 环境的安装包。以下是对常见后缀的详细解读：

### 一、核心结构：`numpy-版本号-<关键标识>.whl`

后缀的核心部分由多个标识组成，用短横线（`-`）分隔，分别对应 **Python 实现、Python 版本、操作系统、硬件架构** 等信息。
...
- **`cp36`、`cp37`、`cp38`、`cp39`**
    ...
    - 前缀 `cp` 代表 **CPython**（官方标准 Python 解释器，最常用的版本）。
    - 后面的数字表示 Python 版本，如 `cp39` 对应 Python 3.9，`cp36` 对应 Python 3.6。
    - 部分标识带 `m`（如 `cp37m`）：`m` 表示该版本编译时启用了 **“ABI with narrow Unicode”**（窄字符集支持），是早期 CPython 的常见标识，不影响日常使用。
- **`pp36`**
    
    - 前缀 `pp` 代表 **PyPy**（一种高性能的 Python 解释器，兼容 CPython 语法但速度更快）。
    - `pp36` 对应 PyPy 3.6 版本。
...
- **`win_amd64`**
    ...
    - 适用于 **64 位 Windows 系统**（`amd64` 泛指 x86-64 架构，兼容 Intel 和 AMD 的 64 位处理器）。
- **`win32`**
    
    - 适用于 **32 位 Windows 系统**。
- **`macosx_10_9_x86_64`**
    
    - 适用于 **macOS 系统**，`10_9` 表示最低支持 macOS 10.9 版本，`x86_64` 对应 64 位 Intel 处理器（注意：部分新版本可能支持 Apple Silicon 芯片，标识可能包含 `arm64`）。
- **`manylinux2010_x86_64`、`manylinux1_x86_64`**
    
    - 适用于 **Linux 系统**，`manylinux` 是 Python 社区定义的兼容标准，确保在多数 Linux 发行版（如 Ubuntu、CentOS）上可运行。
    - `2010` 或 `1` 是标准版本号，`2010` 比 `1` 更晚，兼容性更广（要求 glibc 2.12+）；`x86_64` 对应 64 位处理器。
- **`manylinux2014_aarch64`**
    
    - 适用于 **Linux 系统的 ARM 架构**（如服务器、嵌入式设备的 64 位 ARM 处理器，如 Raspberry Pi 4 64 位系统）。

#### 3. 硬件架构补充

- **`x86_64` / `amd64`**：64 位 x86 架构处理器（主流 PC、服务器常用）。
- **`i686`**：32 位 x86 架构处理器（较旧的设备，如早期笔记本）。
- **`aarch64`**：64 位 ARM 架构（移动设备、嵌入式系统、部分服务器）。

### 三、示例解读

以 `numpy-1.19.5-cp39-cp39-win_amd64.whl` 为例：

  

- `cp39-cp39`：适用于 CPython 3.9 版本。
- `win_amd64`：适用于 64 位 Windows 系统。

### 四、如何选择合适的 `.whl` 文件？

1. 确认你的 **Python 版本**（如 `python --version` 查看，3.9 对应 `cp39`）。
2. 确认你的 **操作系统**（Windows/macOS/Linux）。
3. 确认你的 **硬件架构**（64 位选 `x86_64`/`amd64`，32 位选 `i686`/`win32`，ARM 选 `aarch64`）。

  

如果使用 `pip install` 命令直接安装（不手动下载 `.whl`），pip 会自动根据你的环境选择匹配的版本，无需手动判断。
```
